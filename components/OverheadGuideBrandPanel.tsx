...
import { Brand, TAG_LABELS, TierTag, classify, dragRange, priceLow, weightLow } from "@/lib/overheadGuideData";
import ReelGuideChecklist from "@/components/ReelGuideChecklist";

type SortKey = "price-desc" | "price-asc" | "weight-asc";

const TAG_ORDER: TierTag[] = ["all", "flagship", "lever", "star", "twospeed", "jigging", "levelwind", "biggame", "budget"];

export default function OverheadGuideBrandPanel({ brand, active }: { brand: Brand; active: boolean }) {
        ...
        <div className="flex flex-wrap gap-3 items-center bg-[#1D242B]/[0.03] rounded-[1.5rem] p-3.5 mb-2">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input
              ...
              placeholder="Search series, e.g. Kertang, Dogtooth, SPJ…"
            ...
            {items.map(({ item }) => (
              ...
              >
                ...
                <div className="flex flex-wrap gap-1.5 px-5 pt-3">
                  ...
                  <span className="font-mono text-[10px] bg-[#1D242B] text-white rounded px-1.5 py-0.5 uppercase tracking-wide">
                    {item.dt}
                  </span>
                  {item.dg && (
                  ...
                  <span className="font-mono text-[10px] text-[#1D242B]/50 border border-[#1D242B]/10 rounded px-1.5 py-0.5">{item.sp}</span>
                  {item.lw && (
                    <span className="font-mono text-[10px] text-[#1D242B]/50 border border-[#1D242B]/10 rounded px-1.5 py-0.5">{item.lw}</span>
                  )}
                </div>
