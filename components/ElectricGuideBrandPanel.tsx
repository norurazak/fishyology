...
import { Brand, TAG_LABELS, TAG_ORDER, TierTag, classify, dragRange, priceLow, weightLow } from "@/lib/electricGuideData";
// Reused directly from the spinning reel guide, per AGENTS.md — this component is fully
// generic ({items: ChecklistItem[]}), so there's no reason to duplicate it here.
import ReelGuideChecklist from "@/components/ReelGuideChecklist";

type SortKey = "price-desc" | "price-asc" | "weight-asc";

export default function ElectricGuideBrandPanel({ brand, active }: { brand: Brand; active: boolean }) {
  const [query, setQuery] = useState("");
  const [tag, setTag] = useState<TierTag>("all");
  const [sort, setSort] = useState<SortKey>("price-desc");

  const items = useMemo(() => {
    let list = brand.lineup.map((item) => ({ item, tags: classify(item) }));
    ...
    <section className={active ? "block py-12 md:py-16" : "hidden"}>
      ...
      <div className="mt-12">
        ...
        <p className="text-sm text-[#1D242B]/50 mb-4 mt-1 max-w-xl">
          Search by series, motor or target species – or filter by tier.
        </p>
        ...
        <div className="flex flex-wrap gap-3 items-center bg-[#1D242B]/[0.03] rounded-[1.5rem] p-3.5 mb-2">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input
              ...
              placeholder="Search series, e.g. Kertang, Swordfish, jigging…"
                ...
                <div className="flex flex-wrap gap-1.5 px-5 pt-3">
                  ...
                  {item.dg && (
                    <span className="font-mono text-[10px] text-[#0077C0] border border-[#0077C0]/30 rounded px-1.5 py-0.5">
                      Drag {dragRange(item.dg)}
                    </span>
                  )}
                  {item.mo && (
                    <span className="font-mono text-[10px] text-[#1D242B] bg-[#1D242B]/5 border border-[#1D242B]/15 rounded px-1.5 py-0.5">
                      {item.mo}
                    </span>
                  )}
                  <span className="font-mono text-[10px] text-[#408A71] border border-[#408A71]/30 rounded px-1.5 py-0.5">
                    {item.pw}
                  </span>
                  {item.ej && (
                    <span className="font-mono text-[10px] text-white bg-[#0077C0] rounded px-1.5 py-0.5">
      ...
      <div className="mt-16">
        ...
        <p className="text-sm text-[#1D242B]/50 mb-4 mt-1 max-w-xl">
          Braid (PE) capacity by size class, with a suggested rigging strategy. Electric reels are fished on braid,
          so there&apos;s no mono/fluoro column here.
        </p>
        <div className="bg-white rounded-[1.5rem] border border-[#1D242B]/10 shadow-sm overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full border-collapse min-w-[640px]">
              <thead>
                <tr>
                  {["Size Class", "Braid (PE – Meters)", "Rigging Strategy"].map((h) => (
      ...
      <div className="mt-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
          ...
          <div className="bg-[#1D242B] text-white rounded-[1.5rem] p-6 border-l-4 border-[#0077C0]">
            <h4 className="font-mono text-xs font-black uppercase tracking-widest text-[#C7EEFF] mb-2">Parts &amp; service logistics</h4>
            <p className="text-white/70 text-sm leading-relaxed">{brand.logistics}</p>
          </div>
        </div>
      ...
      {/*
        No suffix-decoder subsection here — see the `suffix` comment on the Brand
        interface in lib/electricGuideData.ts. The source's own buildBrandPanels() never
        renders one for this guide despite each brand carrying suffix data, and the
        reference ReelGuideBrandPanel.tsx this file is modeled on doesn't have this
        subsection either, so omitting it keeps the architecture identical across guides.
      */}
