...
export default function BaitcastGuideQuadrantPlot() {
  ...
  const [pinned, setPinned] = useState<BaitcastPoint | null>(null);
  const [hovered, setHovered] = useState<BaitcastPoint | null>(null);
  ...
  // Braking / frame cross-reference support
  const brStatedCount = POINTS.filter((p) => p.br).length;
  const frStatedCount = POINTS.filter((p) => p.fr).length;
  const magneticCount = POINTS.filter((p) => p.br && /magnetic|magforce|magtrax/i.test(p.br)).length;
  const centrifugalCount = POINTS.filter((p) => p.br && /centrifugal|svs|ivcb|acs/i.test(p.br)).length;
  const dcCount = POINTS.filter((p) => p.br && /\bdc\b|digital|intelligent/i.test(p.br)).length;
  ...
  return (
    <section className="py-16 md:py-24 px-6 md:px-16 max-w-7xl mx-auto border-b border-[#1D242B]/10">
      ...
      <p className="text-[#1D242B]/60 text-base md:text-lg font-medium max-w-2xl mb-8">
        Every series from all six brands plotted on two axes you choose. The crosshair sits at the median of
        ...
        <aside className="bg-white border border-[#1D242B]/10 rounded-[1.5rem] p-5 shadow-sm min-h-[240px] lg:sticky lg:top-24">
          {activePoint ? (
            ...
            <div className="text-sm text-[#1D242B]/50 leading-relaxed">
              ...
              against drag shows what you pay for stopping power; weight against drag shows how much muscle you
              get per gram on your wrist.
            </div>
          )}
        </aside>
      </div>
      ...
      <p className="text-xs text-[#1D242B]/40 leading-relaxed mt-4 max-w-3xl">
        ...
        series. Size class runs 1 (BFS / micro) to 6 (extra heavy) and is normalised across brands.
      </p>
      ...
      <details className="mt-4 bg-white border border-[#1D242B]/10 rounded-[1.5rem] shadow-sm overflow-hidden group">
        <summary className="cursor-pointer px-5 py-4 text-sm font-bold text-[#0077C0] list-none flex items-center gap-2 select-none">
          ...
          Braking, frame &amp; drag cross-reference
        </summary>
        <div className="px-5 pb-5">
          <p className="text-xs text-[#1D242B]/50 leading-relaxed mb-3">
            ...
            Braking and frame material are as stated by the manufacturer where available – a handful of budget
            entries don&apos;t publish either spec.
          </p>
          <div className="overflow-x-auto rounded-xl border border-[#1D242B]/10">
            <table className="w-full border-collapse min-w-[760px]">
              <thead>
                <tr>
                  {["Series", "Brand", "Braking", "Frame", "Max drag", "Size class"].map((h) => (
                ...
                {POINTS.map((p, i) => (
                  <tr key={`${p.brand}-${p.name}`} className={i % 2 === 1 ? "bg-[#1D242B]/[0.02]" : ""}>
                    ...
                    <td className="px-3 py-2 text-sm border-t border-[#1D242B]/5 whitespace-nowrap">
                      {p.br ? <span className="text-[#1D242B]/70">{p.br}</span> : <span className="text-[#1D242B]/35">not stated</span>}
                    </td>
                    <td className="px-3 py-2 text-sm border-t border-[#1D242B]/5 whitespace-nowrap">
                      {p.fr ? <span className="text-[#1D242B]/70">{p.fr}</span> : <span className="text-[#1D242B]/35">not stated</span>}
                    </td>
                    <td className="px-3 py-2 text-sm font-mono font-bold text-[#0077C0] whitespace-nowrap border-t border-[#1D242B]/5">
                      {dragRange(p.dg)}
                    </td>
                    <td className="px-3 py-2 text-sm text-[#1D242B]/70 whitespace-nowrap border-t border-[#1D242B]/5">
                      {AXES.scMax.fmt(p.scMax)}
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <p className="text-xs text-[#1D242B]/40 mt-3 leading-relaxed">
            Of the {POINTS.length} series compared, <strong className="text-[#1D242B]/60">{brStatedCount}</strong>{" "}
            state a braking system and <strong className="text-[#1D242B]/60">{frStatedCount}</strong> state a frame
            material. <strong className="text-[#1D242B]/60">{magneticCount}</strong> run some form of magnetic
            braking (MagTrax, Magforce-Z or similar), <strong className="text-[#1D242B]/60">{centrifugalCount}</strong>{" "}
            use a centrifugal system (SVS, IVCB, ACS or similar), and{" "}
            <strong className="text-[#1D242B]/60">{dcCount}</strong> run a digital or DC braking computer – some
            series combine more than one system across their sizes and are counted in each group that applies.
          </p>
        </div>
      </details>
    </section>
