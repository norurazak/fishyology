...
import { Brand, TAG_LABELS, TierTag, classify, dragRange, priceLow, weightLow } from "@/lib/baitcastGuideData";
import ReelGuideChecklist from "@/components/ReelGuideChecklist";

type SortKey = "price-desc" | "price-asc" | "weight-asc";

const TAG_ORDER: TierTag[] = ["all", "flagship", "bfs", "round", "workhorse", "heavy", "jdm", "budget"];

export default function BaitcastGuideBrandPanel({ brand, active }: { brand: Brand; active: boolean }) {
  ...
  return (
    <section className={active ? "block py-12 md:py-16" : "hidden"}>
        ...
        <div className="flex flex-wrap gap-3 items-center bg-[#1D242B]/[0.03] rounded-[1.5rem] p-3.5 mb-2">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input
              ...
              placeholder="Search series, e.g. Toman, Curado, finesse…"
            ...
            {items.map(({ item }) => (
              ...
              >
                ...
                <div className="flex flex-wrap gap-1.5 px-5 pt-3">
                  ...
                  {item.br && (
                    <span className="font-mono text-[10px] text-[#1D242B]/50 border border-[#1D242B]/10 rounded px-1.5 py-0.5">{item.br}</span>
                  )}
                  {item.fr && (
                    <span className="font-mono text-[10px] text-[#1D242B]/50 border border-[#1D242B]/10 rounded px-1.5 py-0.5">{item.fr}</span>
                  )}
                </div>
      ...
      <div className="mt-16">
        <h3 className="text-xl md:text-2xl font-black uppercase tracking-tight text-[#1D242B] flex items-center gap-2">
          <span className="w-2 h-2 rounded-full bg-[#0077C0]" /> Decoding the suffixes
        </h3>
        <p className="text-sm text-[#1D242B]/50 mb-4 mt-1 max-w-xl">
          What those extra letters and numbers after the series name actually mean.
        </p>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {brand.suffix.map((s) => (
            <div key={s.k} className="bg-white border border-[#1D242B]/10 rounded-xl p-4">
              <code className="font-mono text-sm font-bold text-[#0077C0]">{s.k}</code>
              <p className="text-sm text-[#1D242B]/70 leading-relaxed mt-1.5">{s.v}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="mt-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
          ...
          <div className="bg-[#1D242B] text-white rounded-[1.5rem] p-6 border-l-4 border-[#0077C0]">
            <h4 className="font-mono text-xs font-black uppercase tracking-widest text-[#C7EEFF] mb-2">Parts &amp; logistics</h4>
