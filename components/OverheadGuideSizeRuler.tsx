import { RULER_COLS, RULER_ROWS, TIER_ROWS } from "@/lib/overheadGuideData";

const TIER_BRAND_HEADERS = ["Shimano", "Daiwa", "Abu Garcia", "Penn", "Okuma", "Accurate", "Studio Ocean Mark"];

export default function OverheadGuideSizeRuler() {
  return (
    <section className="py-16 md:py-24 px-6 md:px-16 max-w-7xl mx-auto border-b border-[#1D242B]/10">
      ...
      <p className="text-[#1D242B]/60 text-base md:text-lg font-medium max-w-2xl mb-10">
        The single biggest source of confusion buying across brands: nobody&apos;s &quot;300&quot; is the same reel.
          ...
          <table className="w-full border-collapse min-w-[980px]">
            <thead>
              <tr>
                ...
                {TIER_BRAND_HEADERS.map((h) => (
                  <th
                    key={h}
                    className="text-left px-4 py-3 text-[10px] font-black uppercase tracking-widest text-[#1D242B]/50 border-b border-[#1D242B]/10 whitespace-nowrap"
                  >
                    {h}
                  </th>
                ))}
              ...
              {TIER_ROWS.map((row) => (
                <tr key={row.tier}>
                  ...
                  {row.cells.map((cell, i) => (
                    <td key={i} className="px-4 py-3 text-sm text-[#1D242B]/70 border-t border-[#1D242B]/5 whitespace-nowrap">
                      {cell}
                    </td>
                  ))}
                </tr>
