...
export default function OverheadGuideQuadrantPlot() {
  ...
  const [pinned, setPinned] = useState<OverheadPoint | null>(null);
  const [hovered, setHovered] = useState<OverheadPoint | null>(null);
  ...
  // Drag / speed / line-guide cross-reference support
  const leverCount = POINTS.filter((p) => /lever/i.test(p.dt)).length;
  const starCount = POINTS.filter((p) => /star/i.test(p.dt)).length;
  const twoSpeedCount = POINTS.filter((p) => /2-speed/i.test(p.sp)).length;
  const levelwindCount = POINTS.filter((p) => p.lw && /levelwind/i.test(p.lw)).length;
  const withDrag = POINTS.filter((p) => p.dg);
  const withoutDrag = POINTS.filter((p) => !p.dg);
  const fromGuide = withDrag.filter((p) => /guide/i.test(p.dg!.src));
  const fromSpecSheets = withDrag.filter((p) => !/guide/i.test(p.dg!.src));
  ...
  return (
    <section className="py-16 md:py-24 px-6 md:px-16 max-w-7xl mx-auto border-b border-[#1D242B]/10">
      ...
      <p className="text-[#1D242B]/60 text-base md:text-lg font-medium max-w-2xl mb-8">
        Every series from all seven brands plotted on two axes you choose. The crosshair sits at the median of
              ...
              <button
                ...
                className={`flex items-center gap-2 text-xs font-mono px-3 py-1.5 rounded-full border transition-colors ${
                  off
                    ...
                    : "border-[#1D242B]/15 text-[#1D242B]/70 hover:border-[#0077C0]/40"
                }`}
              >
          ...
          {activePoint ? (
            <div>
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                ...
                <span className="font-mono text-[11px] bg-[#1D242B] text-white rounded-md px-2 py-1 uppercase tracking-wide">
                  {activePoint.dt}
                </span>
                {activePoint.dg && (
                  <span className="font-mono text-[11px] border border-[#0077C0]/30 rounded-md px-2 py-1 text-[#0077C0]">
                    Drag {dragRange(activePoint.dg)}
                  </span>
                )}
                <span className="font-mono text-[11px] border border-[#1D242B]/10 rounded-md px-2 py-1 text-[#1D242B]/70">
                  {activePoint.sp}
              ...
              {activePoint.dg ? (
                <p className="text-[11px] text-[#1D242B]/40 leading-relaxed mb-4">
                  {activePoint.dg.note ?? "Manufacturer-quoted drag ceiling."} —{" "}
                  <span className="text-[#1D242B]/60">{activePoint.dg.src}</span>
                </p>
              ) : (
                <p className="text-[11px] text-[#1D242B]/40 leading-relaxed mb-4">
            ...
            <div className="text-sm text-[#1D242B]/50 leading-relaxed">
              ...
              against weight shows what you pay for lightness; size class against drag shows what you pay for
              pulling power.
            </div>
          )}
        </aside>
      ...
      <p className="text-xs text-[#1D242B]/40 leading-relaxed mt-4 max-w-3xl">
        ...
        series. Size class runs 1 (ultra-light) to 6 (big game / chair) and is normalised across brands, so a
        Penn 30 and a Shimano Tiagra 30A both plot as the same class.
      </p>
      ...
      <details className="mt-4 bg-white border border-[#1D242B]/10 rounded-[1.5rem] shadow-sm overflow-hidden group">
        <summary className="cursor-pointer px-5 py-4 text-sm font-bold text-[#0077C0] list-none flex items-center gap-2 select-none">
          ...
          Drag, speed &amp; line guide cross-reference
        </summary>
        <div className="px-5 pb-5">
          ...
          <div className="overflow-x-auto rounded-xl border border-[#1D242B]/10">
            <table className="w-full border-collapse min-w-[760px]">
              <thead>
                <tr>
                  {["Series", "Brand", "Drag type", "Max drag", "Drag source", "Speeds", "Line guide"].map((h) => (
              ...
              <tbody>
                {POINTS.map((p, i) => (
                  <tr key={`${p.brand}-${p.name}`} className={i % 2 === 1 ? "bg-[#1D242B]/[0.02]" : ""}>
                    ...
                    <td className="px-3 py-2 text-sm text-[#1D242B]/70 whitespace-nowrap border-t border-[#1D242B]/5">{p.dt}</td>
                    <td className="px-3 py-2 text-sm font-mono font-bold text-[#0077C0] whitespace-nowrap border-t border-[#1D242B]/5">
                      {p.dg ? dragRange(p.dg) : "not stated"}
                    </td>
                    <td className="px-3 py-2 text-xs text-[#1D242B]/60 whitespace-nowrap border-t border-[#1D242B]/5">{p.dg?.src ?? "—"}</td>
                    <td className="px-3 py-2 text-sm text-[#1D242B]/70 whitespace-nowrap border-t border-[#1D242B]/5">{p.sp}</td>
                    <td className="px-3 py-2 text-sm border-t border-[#1D242B]/5 whitespace-nowrap">
                      {p.lw ? (
                        <span className="text-[#1D242B]/70">{p.lw}</span>
                      ) : (
                        <span className="text-[#1D242B]/35">not stated</span>
                      )}
                    </td>
                  </tr>
                ))}
          ...
          <p className="text-xs text-[#1D242B]/40 mt-3 leading-relaxed">
            Of the {POINTS.length} series compared, {leverCount} offer a lever drag and {starCount} a star drag
            (a handful of Daiwa and Shimano hybrids carry both). {twoSpeedCount} run a 2-speed gearbox, and{" "}
            {levelwindCount} carry a levelwind carriage rather than an open top. {withDrag.length} of {POINTS.length}{" "}
            series carry a confirmed max-drag figure
            {withoutDrag.length > 0 && <> ({withoutDrag.length} not yet confirmed)</>}. Of those {withDrag.length},{" "}
            {fromGuide.length} came straight from the brand guide&apos;s own column, and {fromSpecSheets.length} were
            read off manufacturer or dealer spec sheets for Penn, which has no source guide of its own – the other
            six brands are all sourced from the brand comparison guide (supplied).
          </p>
        </div>
      </details>
    </section>
